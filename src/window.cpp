...
#endif
#if !defined(DOKUN_USE_OPENGL) || !defined(DOKUN_USE_VULKAN)
...
if(Renderer::get_current_API() == "OpenGL") {
  ...
  XStoreName(display, window, title.c_str()); // set window title
   ...
   // create a vulkan window
...
#ifdef DOKUN_USE_GLFW // library already initialized with glfwInit() in Engine::on_open()
    ...
    if (!window) {
        std::cerr << "Failed to open GLFW window. Probably due to hardware or drivers not being OpenGL 3.3 compatible..\n";
        glfwTerminate();
    }
...
#ifdef __gnu_linux__
#ifdef DOKUN_X11
#ifndef DOKUN_USE_OPENGL
#ifndef DOKUN_USE_VULKAN
    XFreeGC(display, gc);
#endif    
#endif
#ifdef DOKUN_USE_OPENGL
...
////////////
#ifdef DOKUN_USE_GLFW
GLFWwindow * dokun::Window::get_GLFW_window() const { 
    return window; 
}
////////////
void dokun::Window::framebuffer_resize_callback(GLFWwindow *window, int width, int height) {
    auto dokun_window = reinterpret_cast<dokun::Window *>(glfwGetWindowUserPointer(window));
    //dokun_window->framebufferResized = true;
    dokun_window->width = width;
    dokun_window->height = height;
    std::cout << "window resized (framebuffer_callback): " << Vector2i(dokun_window->width, dokun_window->height) << std::endl;
}
#endif
////////////
// XSetWindowBorderWidth (display, w, width)
...
#endif
#ifdef DOKUN_USE_GLFW
    if(glfwCreateWindowSurface(Renderer::get_pointer()->instance, window, nullptr, surface) != VK_SUCCESS) {
